 ...
 */
export const MODULE_EXTENSIONS = ['.js', '.mjs', '.ts', '.mts']
...
export function isModuleFile(filename) {
  if (/\.(test|spec)\.(m?[jt]s)$/.test(filename)) return false

  return MODULE_EXTENSIONS.some((ext) => filename.endsWith(ext))
}
 ...
 * @returns {string}
 */
export function stripModuleExtension(filename) {
  return filename.replace(/\.(m?[jt]s)$/, '')
}
